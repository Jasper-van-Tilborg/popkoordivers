...
function resolveImageUrl(item: MediaItem): string | null {
  if (item.r2_key) return getPublicUrl(item.r2_key);
  if (item.url && !item.url.startsWith("placeholder")) return item.url;
  return null;
}
      ...
      <main style={{ minHeight: "100vh", background: "#FFFFFF" }}>
        ...
        <section className="page-hero" style={{ background: "var(--primary)", padding: "140px 24px 100px", minHeight: "420px", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ maxWidth: "1200px", margin: "0 auto", position: "relative", zIndex: 1 }}>
            ...
            <RevealText text="Foto's & Video's" as="h1" pageDelay={0} style={{ fontSize: "clamp(36px, 5vw, 60px)", fontWeight: 800, letterSpacing: "-2px", lineHeight: 1.1, color: "#FFFFFF", margin: "0 0 24px" }} />
            <Reveal delay={180}>
        ...
        <section style={{ padding: "60px 24px 120px", background: "#FFFFFF" }}>
          <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
            {items.length === 0 && albums.every((a) => a.photos.length === 0) ? (
              <Reveal>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "16px" }} className="media-grid">
                {albums.filter((a) => a.photos.length > 0).map((album, i) => (
                  <Reveal key={album.prefix} delay={Math.min(i % 3, 2) * 60}>
                    <AlbumCard slug={album.slug} title={album.title} datum={album.datum} photos={album.photos} />
                  </Reveal>
                ))}
                {items.map((item, i) => (
                  <Reveal key={item.id} delay={Math.min((i + albums.filter((a) => a.photos.length > 0).length) % 3, 2) * 60}>
                    {item.type === "video" ? (
                      <VideoCard item={item} />
                    ) : (
...
function FotoCard({ item, gradient }: { item: MediaItem; gradient: string }) {
  ...
  return (
    <div style={{ borderRadius: "16px", overflow: "hidden", border: "1px solid rgba(0,0,0,0.07)", boxShadow: "0 2px 8px rgba(0,0,0,0.04)", transition: "transform 0.2s, box-shadow 0.2s", cursor: "pointer" }} className="card-hover">
      <div style={{ aspectRatio: "4/3", background: imageUrl ? undefined : gradient, overflow: "hidden", position: "relative" }}>
        {imageUrl && (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={imageUrl} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block", transition: "transform 0.4s ease" }} />
        )}
      </div>
      <ItemMeta item={item} />
    </div>
...
function VideoCard({ item }: { item: MediaItem }) {
  const vid = item.url ? youtubeId(item.url) : null;
  return (
    <div style={{ borderRadius: "16px", overflow: "hidden", border: "1px solid rgba(0,0,0,0.07)", boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
      <div style={{ position: "relative", aspectRatio: "16/9" }}>
        {vid ? (
          <iframe src={`https://www.youtube.com/embed/${vid}`} title={item.label} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen style={{ width: "100%", height: "100%", border: "none", display: "block" }} />
        ) : (
          <div style={{ width: "100%", height: "100%", background: "linear-gradient(135deg, rgba(243,106,42,0.12) 0%, #FDE8D8 100%)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--primary)" }}>
            <IconPlay size={32} />
          </div>
        )}
      </div>
...
function ItemMeta({ item }: { item: MediaItem }) {
  return (
    <div style={{ padding: "14px 16px", background: "#FFFFFF" }}>
      <p style={{ margin: 0, fontSize: "14px", fontWeight: 700, color: "#111", lineHeight: 1.3 }}>{item.label}</p>
      {item.datum && <p style={{ margin: "4px 0 0", fontSize: "12px", color: "#aaa", fontWeight: 500 }}>{item.datum}</p>}
    </div>
  );
}
